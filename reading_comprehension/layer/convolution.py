...
__all__ = ["Conv1D", "Conv2D", "MultiConv1D", "MultiConv2D",
           ...
           "Conv", "MultiConv", "SeparableConv", "MultiSeparableConv"
...
class MultiConv(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE), tf.device('/CPU:0'):
            ...
            for i in range(len(self.window_size)):
                layer_scope = "window_{0}".format(i)
                layer_default_gpu_id = self.default_gpu_id + i if self.enable_multi_gpu == True else self.default_gpu_id
                conv_layer = Conv(num_channel=self.num_channel, num_filter=self.num_filter,
                    window_size=self.window_size[i], stride_size=self.stride_size, padding_type=self.padding_type,
                    activation=self.activation, dropout=self.dropout, layer_dropout=self.layer_dropout, layer_norm=self.layer_norm,
                    residual_connect=self.residual_connect, num_gpus=self.num_gpus, default_gpu_id=layer_default_gpu_id,
                    regularizer=self.regularizer, random_seed=self.random_seed, trainable=self.trainable, scope=layer_scope)
    ...
    def __call__(self,
                 input_data,
                 input_mask):
        """call multi-window convolution layer"""
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE), tf.device(self.device_spec):
            input_conv_list = []
            input_conv_mask_list = []
            for conv_layer in self.conv_layer_list:
...
class SeparableConv(object):
    ...
    def __call__(self,
                 input_data,
                 input_mask):
        """call depthwise-separable convolution layer"""
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE), tf.device(self.device_spec):
            input_data_shape = tf.shape(input_data)
            input_mask_shape = tf.shape(input_mask)
            shape_size = len(input_data.get_shape().as_list())
...
class MultiSeparableConv(object):
    """multi-window depthwise-separable convolution layer"""
    def __init__(self,
                 num_channel,
                 num_filter,
                 num_multiplier,
                 ...
                 random_seed=0,
                 trainable=True,
                 scope="multi_sep_conv"):
        """initialize multi-window depthwise-separable convolution layer"""
        self.num_channel = num_channel
        self.num_filter = num_filter
        self.num_multiplier = num_multiplier
        self.window_size = window_size
        ...
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE), tf.device('/CPU:0'):
            ...
            for i in range(len(self.window_size)):
                layer_scope = "window_{0}".format(i)
                layer_default_gpu_id = self.default_gpu_id + i if self.enable_multi_gpu == True else self.default_gpu_id
                conv_layer = SeparableConv(num_channel=self.num_channel, num_filter=self.num_filter, num_multiplier=self.num_multiplier,
                    window_size=self.window_size[i], stride_size=self.stride_size, padding_type=self.padding_type,
                    activation=self.activation, dropout=self.dropout, layer_dropout=self.layer_dropout, layer_norm=self.layer_norm,
                    residual_connect=self.residual_connect, num_gpus=self.num_gpus, default_gpu_id=layer_default_gpu_id,
                    regularizer=self.regularizer, random_seed=self.random_seed, trainable=self.trainable, scope=layer_scope)
